...
In some cases editing the contribution limits can be an `O(n)` operation. Adding a whitelist is an `O(n)` operation. Modifying an existing whitelist is an `O(w)` operation where `w` is the sum of additions and deletions on the whitelist.